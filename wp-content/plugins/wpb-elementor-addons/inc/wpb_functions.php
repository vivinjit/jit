...
function wpb_ea_pro_required_addon( $addons ){
    ...
    $pro_addons = apply_filters( 'wpb_ea_pro_required_addon', 
        array(
            array(
                'name'      => WPB_EA_PREFIX . 'pro_addons',
                'label'     => esc_html__( 'Pro Elements', 'wpb-elementor-news-ticker-pro' ),
                'type'      => 'section_title',
            )
        )
    );
...
function wpb_nt_pro_addons_link( $addons ){
    ...
    if( !defined( 'WPB_NT_VERSION' ) ){
        $addons[] = array(
            ...
            'label'     => esc_html__( 'News Ticker PRO', 'wpb-elementor-news-ticker-pro' ),
